...
pygame.display.set_caption('Weeping Willow 1.1')
done = False
...
def level1():
    ...
    while seconds <= 30:
        ...
        screen.blit(timetext, [650, 5])

        if livesminus > 0:
            loselife = font.render("-1", True, RED)
            screen.blit(loselife, [130, 30])
            livesminus -= 1

        if livesadd > 0:
            gainlife = font.render("+1", True, GREEN)
            screen.blit(gainlife, [130, 30])
            livesadd -= 1

        if scoreadd > 0:
            gainscore = font.render("+1", True, GREEN)
            screen.blit(gainscore, [130, 5])
            scoreadd -= 1
...
def boss1():
    ...
    while not answered:
        ...
        choicetext = font.render("Press S to skip, or press C to continue!", True, WHITE)
        screen.blit(choicetext, [50, 200])

        choicetext = font.render("Boss incoming!", True, WHITE)
        screen.blit(choicetext, [250, 100])
...
def boss2():
    ...
    if not skipped:
        ...
        #Short pause
        textList4 = ["...", "You didn't fight him?", "How'd you get past him then?!"]
        #short pause
    ...
    if not skipped:
        ...
        for k in range(int((SCREEN_WIDTH-(bubbleAttack.rect.x + bubbleAttack.sizex)-(firelord.sizex))/5)):
            ...
            if bubbleAttack.rect.x >= firelord.rect.x:
                bubbleAttack.setcoords(bubbleAttack.rect.x+10, bubbleAttack.rect.y)
                firelord.setcoords(firelord.rect.x+10, firelord.rect.y)
                if bubbleAttack.rect.x % 5 == 0:
                    bubbleList.draw(screen)
            else:
